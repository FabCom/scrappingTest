...
def scrapp_crypto
  ...
  page.xpath("//tr[contains(@class,'cmc-table-row')]//td[contains(@class,'cmc-table__cell--sort-by__name')]//a[2]").each do |current|
    crypto_name_array.push(current.text)
  end
  page.xpath("//tr[contains(@class,'cmc-table-row')]//td[contains(@class,'cmc-table__cell--sort-by__price')] ").each do |current|
    crypto_values_array.push(current.text)
  end
...
def process
  print "\n" * 2
  puts "#{' ' * 20}CRYPTO SCRAPPER"
  print "\n" * 2
  puts 'Tu veux du Hash :'
  puts scrapp_crypto
  print "\n" * 2
  puts 'Tu veux du Array :'
